...
public class HelloService {
	...
	public String Hello() {
		try {   	        
			...
			request.put("name", "lh");
			request.put("pwd", "12345");
			String url = "http://PROVIDER-SERVICE/providerForObject?name= {name}&pwd= {pwd}";
			Long startTime=System.currentTimeMillis();
			Map map1 = restTemplate.getForObject(url, Map.class, request);
			System.err.println("=====providerForObject 总耗时:"+(System.currentTimeMillis()-startTime)+"ms");
			ResponseEntity<Map> entity = restTemplate.getForEntity(url, Map.class, request);
			...
			return "Hello,Customer! msg from providerForObject : <br/><br/> " + map1.get("name") + ","
					+ map1.get("pwd");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public String HelloFallBack(){
		String msg = "HelloService触发Hystrix熔断器，调用降级服务";
